...
public abstract class OCSInputBase {
    public abstract byte id { get; }
}

public abstract class OCSInputSender : OCSInputBase {
    public abstract void PendInputsPerFrame(OCSInputStream inputStream);
}

public abstract class OCSInputStream {
    private bool _streaming = false;
    private OCSTicker _ticker;

    protected Dictionary<byte, OCSInputSender> senders { get; private set; }

    public OCSInputStream() {
        _ticker = new OCSTicker();

        senders = new Dictionary<byte, OCSInputSender>();
    }

    public void RegisterInputSender(OCSInputSender sender) {
        senders[sender.id] = sender;
    }

    public void UnregisterInputSender(OCSInputSender sender) {
    ...
    public void PendState(OCSInputSender sender, byte control, byte state) {
        PendStateImpl(sender.id, control, state);
    }

    public void PendByteAxis(OCSInputSender sender, byte control, byte axis) {
        PendByteAxisImpl(sender.id, control, axis);
    }

    public void PendAxis(OCSInputSender sender, byte control, float axis) {
        PendAxisImpl(sender.id, control, axis);
    }

    public void PendAxis2D(OCSInputSender sender, byte control, Vector2 axis2D) {
        PendAxis2DImpl(sender.id, control, axis2D);
    }

    public void PendPose(OCSInputSender sender, byte control, Vector3 position, Quaternion rotation) {
        PendPoseImpl(sender.id, control, position, rotation);
    }

    public void PendRaycastHit(OCSInputSender sender, byte control, Vector3 origin, Vector3 hitPosition, Vector3 hitNormal) {
    ...
    public bool IsActive(byte device, byte control, OCSInputDirection direction) {
    ...
    public bool GetActivated(byte device, byte control, OCSInputDirection direction) {
    ...
    public bool GetDeactivated(byte device, byte control, OCSInputDirection direction) {
    ...
    protected abstract bool IsActiveImpl(byte device, byte control, OCSInputDirection direction);
    protected abstract bool GetActivatedImpl(byte device, byte control);
    protected abstract bool GetActivatedImpl(byte device, byte control, OCSInputDirection direction);
    protected abstract bool GetDeactivatedImpl(byte device, byte control);
    protected abstract bool GetDeactivatedImpl(byte device, byte control, OCSInputDirection direction);
