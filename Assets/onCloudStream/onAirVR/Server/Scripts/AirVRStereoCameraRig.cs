...
public sealed class AirVRStereoCameraRig : AirVRCameraRig, IAirVRTrackingModelContext {
    ...
    protected override void onAwake() {
        switch (_eventSystemResponsive) {
            case HandSelect.Left:
                prepareForEventSystem(leftHandAnchor.gameObject.AddComponent<AirVRPointer>(), OCSInputDeviceID.LeftHandTracker);
                break;
            case HandSelect.Right:
                prepareForEventSystem(rightHandAnchor.gameObject.AddComponent<AirVRPointer>(), OCSInputDeviceID.RightHandTracker);
                break;
    ...
    private void prepareForEventSystem(AirVRPointer pointer, OCSInputDeviceID srcDevice) {
    ...
    protected override void updateControllerTransforms(AirVRClientConfig config) {
        var pose = inputStream.GetPose((byte)OCSInputDeviceID.LeftHandTracker, (byte)OCSHandTrackerControl.Pose);
        leftHandAnchor.localPosition = pose.position;
        leftHandAnchor.localRotation = pose.rotation;

        pose = inputStream.GetPose((byte)OCSInputDeviceID.RightHandTracker, (byte)OCSHandTrackerControl.Pose);
